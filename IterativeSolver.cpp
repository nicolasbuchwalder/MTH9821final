    ...
    case StoppingCriterion::consecutive:
    {
        ...
        while (diff.norm() > tolerance)
        {
            xnew[0] = std::max(bound[0],
                ((1. - omega_) * xold[0]) + ((omega_ * alpha) / (2. * (1. + alpha)) * (xleft + xold[0 + 1])) + (omega_ / (1. + alpha) * b[0]));
            for (std::size_t pos = 1; pos < N - 1; pos++)
            {
                xnew[pos] = std::max(bound[pos],
                    ((1. - omega_) * xold[pos]) + ((omega_ * alpha) / (2. * (1. + alpha)) * (xnew[pos - 1] + xold[pos + 1])) + (omega_ / (1. + alpha) * b[pos]));
            }
            xnew[N - 1] = std::max(bound[N - 1],
                    ((1. - omega_) * xold[N - 1]) + ((omega_ * alpha) / (2. * (1. + alpha)) * (xnew[N - 1 - 1] + xright)) + (omega_ / (1. + alpha) * b[N - 1]));
            diff = xnew - xold;
            xold = xnew;
            iter_N++;
        }
